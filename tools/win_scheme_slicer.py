...
Windows-friendly: положить в папку с изображениями и запустить без параметров.

Использование:
    python scheme_slicer.py
    python scheme_slicer.py --tile-size 1536 --overlap 64 --clean
    python scheme_slicer.py "C:\\Pictures\\Schemes" --output "D:\\Sliced"
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Нарезка схем: превью, фрагменты с overlap, чистые фрагменты, индекс. "
                    "Положите скрипт в папку с изображениями и запустите без параметров."
    )
    parser.add_argument("input", nargs="?", default=None,
                        help="Директория с исходными изображениями (по умолчанию — папка со скриптом)")
    parser.add_argument("-o", "--output", default=None,
                        help="Выходная директория (по умолчанию — 'sliced_output' рядом со скриптом)")
    parser.add_argument("--use-cwd", action="store_true",
                        help="Использовать текущую рабочую директорию вместо папки со скриптом")
    ...
    if args.input:
        ...
        in_dir = script_path.parent

    # --- Определяем выходную директорию ---
    if args.output:
        out_dir = Path(args.output)
    else:
        out_dir = in_dir / "sliced_output"
    ...
    print(f"Входная папка : {in_dir.resolve()}")
